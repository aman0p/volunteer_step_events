...
export default function EventEnrollmentTable({
   ...
   const paginatedData = enrollments.slice(
      (page - 1) * rowsPerPage,
      page * rowsPerPage
   );
      ...
      try {
         const promises = selectedRows.map(async (id) => {
            ...
            if (result.success) {
               ...
               toast.error(
                  result.message || `Failed to approve enrollment ${id}`
               );
            }
         });
         await Promise.all(promises);
         toast.success(
            `Successfully processed ${selectedRows.length} enrollment(s)`
         );
         setSelectedRows([]);
      ...
      try {
         const promises = selectedRows.map(async (id) => {
            ...
            if (result.success) {
               ...
               toast.error(
                  result.message || `Failed to reject enrollment ${id}`
               );
            }
         });
         await Promise.all(promises);
   ...
   const isAllSelected =
      selectedRows.length === enrollments.length && enrollments.length > 0;
