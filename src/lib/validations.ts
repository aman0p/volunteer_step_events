      ...
      eventRoles: z
         .array(
            z.object({
               ...
               payout: z
                  .number()
                  .min(0, "Payout must be non-negative")
                  .max(1000000, "Payout cannot exceed ₹10,00,000"),
               maxCount: z
                  .number()
                  .int()
                  .min(1, "Max count must be at least 1")
                  .max(100, "Max count cannot exceed 100"),
            })
         )
         .optional(),
      quickLinks: z
   ...
   .refine(
      (data) => {
         if (data.eventRoles && data.eventRoles.length > 0) {
            ...
            const roleNames = data.eventRoles.map((role) =>
   ...
   .refine(
      (data) => {
         if (data.quickLinks && data.quickLinks.length > 0) {
            ...
            const linkTitles = data.quickLinks.map((link) =>
...
export const profileSchema = z.object({
   ...
   skills: z
      .array(
         z.string().trim().min(2, "Each skill must be at least 2 characters")
      )
      .max(10)
      .optional(),
